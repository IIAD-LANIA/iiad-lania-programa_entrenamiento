...
# HELPER TEMA — PLOTLY
# ─────────────────────────────────────────────────────────────────────────────
def plotly_layout_base(dark: bool = True):
    """Layout Plotly adaptado al tema.

    Pasa dark=False para modo claro. Por defecto dark=True porque el CSS de la
    app fuerza fondo oscuro en la sidebar. Llama con dark=st.session_state.get(
    'dark_mode', True) si quieres que responda al toggle de tema.
    """
    if dark:
        bg      = "rgba(0,0,0,0)"           # transparente — hereda el fondo oscuro
        plot_bg = "rgba(20,24,40,0.80)"     # azul-noche semi-transparente
        font_c  = "#E8EDF3"
        tick_c  = "#B0BEC5"
        grid_c  = "rgba(255,255,255,0.07)"
        line_c  = "rgba(255,255,255,0.12)"
        leg_bg  = "rgba(0,0,0,0)"
        leg_border = "rgba(255,255,255,0.08)"
    else:
        bg      = "rgba(0,0,0,0)"           # transparente — hereda el fondo claro
        plot_bg = "rgba(247,249,252,0.95)"  # gris muy claro
        font_c  = "#1E293B"
        tick_c  = "#475569"
        grid_c  = "rgba(0,0,0,0.06)"
        line_c  = "rgba(0,0,0,0.10)"
        leg_bg  = "rgba(255,255,255,0.85)"
        leg_border = "rgba(0,0,0,0.08)"
    return dict(
        plot_bgcolor=plot_bg,
        paper_bgcolor=bg,
        font=dict(color=font_c, family="Inter, Segoe UI, sans-serif", size=12),
        xaxis=dict(
            gridcolor=grid_c,
            linecolor=line_c,
...
def _dark() -> bool:
    """Detecta si el tema activo es oscuro consultando session_state o config."""
    # Prioridad 1: toggle manual en session_state
    if "dark_mode" in st.session_state:
        return bool(st.session_state["dark_mode"])
    # Prioridad 2: st.get_option (funciona en local, falla en Cloud)
    try:
        return st.get_option("theme.base") != "light"
    except Exception:
        return True  # fallback seguro: modo oscuro
# ─────────────────────────────────────────────────────────────────────────────
# ESTILOS CSS
# ─────────────────────────────────────────────────────────────────────────────
def inject_css():
    ...
    # ── CSS extra: modo oscuro en iframes Plotly, tablas y widgets ──────────
    st.markdown("""
<style>
/* Plotly chart containers: fondo transparente */
.stPlotlyChart > div,
...
.stDataFrame tbody tr:nth-child(even) { background-color: #131929 !important; }
.stDataFrame tbody tr:nth-child(odd)  { background-color: #0E1117 !important; }
.stDataFrame tbody td { color: #CFD8DC !important; }
/* Labels de inputs */
...
/* st.metric() */
[data-testid="stMetricValue"]  { color: #E8EDF3 !important; }
...
def pagina_dashboard():
    ...
    with col_left:
        ...
        fig.add_vline(
            x=60, line_dash="dash", line_color="#FFC107",
            annotation_text="Meta Intermedia 60%",
            annotation_font_color="#FFC107",
            annotation_font_size=11,
            annotation_bgcolor="rgba(13,27,42,0.75)",
        )
        fig.add_vline(
            x=100, line_dash="dash", line_color="#66BB6A",
    ...
    with col_right:
        ...
        _base = plotly_layout_base(_dark())
        # Fusionamos legend del base con las propiedades específicas del pie
        # para evitar el argumento duplicado que causa TypeError.
        _base["legend"] = dict(
...
def pagina_analisis_rol():
    ...
    if not df_res.empty:
        ...
        fig.add_hline(
            y=60, line_dash="dash", line_color="#FFC107",
            annotation_text="Meta intermedia 60%",
            annotation_font_color="#FFC107",
            annotation_font_size=11,
            annotation_bgcolor="rgba(13,27,42,0.75)",
        )
        fig.update_traces(texttemplate="%{text:.1f}%", textposition="outside", textfont=dict(color="#E8EDF3", size=11))
        fig.update_layout(
...
def pagina_cronograma():
    ...
    st.subheader("📊 Distribución de Horas por Mes")
    fig = px.bar(meses_horas, x="MesNom", y="Horas",
                 color="Horas", color_continuous_scale="Blues", text="Horas",
                 labels={"MesNom": "Mes", "Horas": "Horas"})
    fig.update_traces(
        texttemplate="%{text:.0f}h",
        textposition="outside",
        textfont=dict(color="#E8EDF3", size=12),
    )
    _layout = plotly_layout_base(_dark())
    _layout["coloraxis_colorbar"] = dict(
        title="Horas",
